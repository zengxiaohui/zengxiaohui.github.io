var fsBanner = function(container,options) {
	...
	this.setup = function() {
		...
		this.part = this.container.width() / this.items.length -5;
		this.mini = this.part/2;
		this.widmain = this.container.width() - (this.mini*this.items.length-1);
		// console.log(this.mini,'252525',this.part)
		...
		if (iexpanded == this.ilast) {
			...
			this.items.each(function(i) {
				...
				if (i <= iexpanded) {
					...
					$item.stop().animate({'left':i*self.mini+self.widmain - $(container).height()*0.1});
				}
				if (self.options.showName) {
					var $name = $item.find('.name');
