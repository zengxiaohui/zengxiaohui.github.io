...
+ function($) {
    "use strict";
     jQuery.getScript("/global/plugins/leancloud/av-mini.js", function(data, status, jqxhr) {
        ...
        Fileinput.prototype.change = function(e) {
            ...
            if (this.$preview.length > 0 && (typeof file.type !== "undefined" ? file.type.match(/^image\/(gif|png|jpeg)$/) : file.name.match(/\.(gif|png|jpe?g)$/i)) && typeof FileReader !== "undefined") {
                ...
                avFile.save().then(function(data) {
                    ...
                    $img[0].src = data.attributes.url;
                    files[0].result = data.attributes.url;
                }, function(error) {
                    // The file either could not be read, or could not be saved to AV.
                });
                // var form = new FormData();

          /*  form.append("author", "zhangman");                        // 可以增加表单数据
            form.append("file", file);                           // 文件对象
            // XMLHttpRequest 对象
            var xhr = new XMLHttpRequest();
            xhr.open("post", 'http://120.55.186.105:8080/upload', true);
            ...
            xhr.onload = function () {
                ...
                console.log(res);
                $img[0].src = res.data.urlHttp;
                files[0].result = res.data.urlHttp;
            };
            xhr.send(form);*/
                    //   $img[0].src = temp
                    // files[0].result = temp
